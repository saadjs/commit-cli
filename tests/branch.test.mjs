...
test('generated names are normalized, bounded, and have a usable fallback', () => {
  ...
  for (const value of [undefined, '', '💥', '---']) assert.match(sanitizeBranchName(value), /^commit-\d+$/);
});
...
test('response parsing preserves agent branch and tolerates missing/malformed branch fields', () => {
  for (const branch of ['agent-chosen', undefined, 123, null, {}]) {
    const response = JSON.stringify({ branch, commits: [{ subject: ' feat: something ', body: 'why' }] });
    assert.equal(parseResponse('```json\n' + response + '\n```').branch, typeof branch === 'string' ? branch : undefined);
    assert.deepEqual(parseResponse(response).commits, parseCommits(response));
  }
  for (const raw of ['', '{}', '{"commits":[null]}', '{"commits":[{"subject":""}]}']) assert.throws(() => parseResponse(raw));
});
test('branch schema and naming instruction are requested only when needed in either mode', () => {
  for (const split of [false, true]) for (const wantBranch of [false, true]) {
    const prompt = buildPrompt({ branch: 'main', files: [], stat: '', diff: '', truncated: false, recentSubjects: [], split, wantBranch });
    const schema = JSON.parse(prompt.split('Output schema:\n')[1].split('\n\n')[0]);
