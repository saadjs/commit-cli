...
function fixture(t, initial = false) {
  ...
  script(
    ...
    if (args[0] === 'repo' && process.env.REPO_RESPONSE) { console.log(process.env.REPO_RESPONSE); process.exit(0); }
    if (args[1] === 'list' && process.env.PR_RESPONSE) { console.log(process.env.PR_RESPONSE); process.exit(0); }
  ...
  /** @param {string[]} args @param {NodeJS.ProcessEnv} extra */
...
test("clean branch pushes only itself and sets upstream", (t) => {
  ...
  assert.match(
    readFileSync(join(f.dir, "pushes"), "utf8"),
    /refs\/heads\/feature:refs\/heads\/feature/,
  );
});

for (const flag of ["--push", "--pr"])
  test(`${flag} does not publish tags when push.followTags is enabled`, (t) => {
    const f = fixture(t);
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.equal(
      f.git("--git-dir", f.remote, "rev-parse", "refs/heads/feature"),
...
test("new commits are created before pushing and PR creation", (t) => {
  const f = fixture(t);
  writeFileSync(join(f.cwd, "file.txt"), "base\nfeature\nmore\n");
  ...
  assert.match(logs, /--draft/);
  assert.match(logs, /--body-file/);
  assert.match(logs, /--head/);
});
...
test("existing PR pushes without generating or changing its description", (t) => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /pull\/1/);
...
test("dry run previews pending changes without creating branch, commit, push, or PR", (t) => {
  const f = fixture(t);
  writeFileSync(join(f.cwd, "file.txt"), "pending unique change\n");
  const before = f.git("rev-parse", "HEAD");
...
test("failed push preserves commits and prevents PR creation", (t) => {
  ...
  assert.match(result.stderr, /Local commits are preserved/);
  assert.equal(f.git("rev-parse", "HEAD"), before);
  assert.equal(existsSync(join(f.dir, "body")), false);
});
...
test("PR creation failure preserves pushed branch and retry needs no new commit", (t) => {
  ...
  assert.match(failed.stderr, /branch was pushed/);
...
for (const { name, extra, pattern } of [
  { name: "fork", extra: { FORK: "1" }, pattern: /fork destinations/ },
  { name: "authentication", extra: { FAIL_AUTH: "1" }, pattern: /auth status/ },
])
...
test("PR prompt includes full committed branch and template, excludes worktree edits", async (t) => {
  ...
  const prompt = await prPrompt(
    f.cwd,
    { remote: "origin", pushUrl: f.remote, baseSha: f.git("rev-parse", "main"), base: "main" },
    "feature",
    200000,
    false,
  );
  assert.match(prompt, /\+feature/);
  assert.match(prompt, /## Required section/);
  assert.doesNotMatch(prompt, /WORKTREE_ONLY/);
});
...
test("failed commit hook stops publishing and leaves staged work on new branch", (t) => {
  ...
  assert.notEqual(f.git("diff", "--cached"), "");
  assert.equal(existsSync(join(f.dir, "pushes")), false);
});

test("explicit remote is used when origin is absent", (t) => {
  const f = fixture(t);
  f.git("remote", "rename", "origin", "publish");
...
test("invalid flag combinations and ambiguous remotes stop before staging", (t) => {
  ...
  const result = f.run(["--push", "-y"]);
  assert.equal(result.status, 1);
  assert.match(result.stderr, /multiple push URLs/);
});
...
test("PR editor preserves Markdown headings and editor failure cancels", (t) => {
  ...
  const result = spawnSync(process.execPath, ["--input-type=module", "-e", source], {
    env: { ...process.env, GIT_EDITOR: `${process.execPath} ${editor}` },
    encoding: "utf8",
  });
  assert.equal(result.status, 0, result.stderr);
  assert.match(JSON.parse(result.stdout), /## Summary/);
...
test("default PR template takes precedence over multiple optional templates", async (t) => {
  const f = fixture(t);
  mkdirSync(join(f.cwd, ".github/PULL_REQUEST_TEMPLATE"), { recursive: true });
  for (const name of ["bug.md", "feature.md"])
    writeFileSync(join(f.cwd, ".github/PULL_REQUEST_TEMPLATE", name), name);
  const destination = {
    remote: "origin",
    pushUrl: f.remote,
  ...
  await assert.rejects(
    prPrompt(f.cwd, destination, "feature", 200000, false),
...
for (const extra of [
  { REPO_RESPONSE: '{"nameWithOwner":12,"isFork":false,"defaultBranchRef":{"name":"main"}}' },
  {
    REPO_RESPONSE:
      '{"nameWithOwner":"owner/repo","isFork":"false","defaultBranchRef":{"name":"main"}}',
  },
  {
    PR_RESPONSE:
      '[{"url":"https://github.com/owner/repo/pull/1","isCrossRepository":"false","baseRefName":"main"}]',
  },
  { PR_RESPONSE: '[{"url":12,"isCrossRepository":false,"baseRefName":"main"}]' },
]) {
  test(`malformed GitHub response stops publishing: ${JSON.stringify(extra)}`, (t) => {
    const f = fixture(t);
    const before = f.git("rev-parse", "HEAD");
    if ("REPO_RESPONSE" in extra) writeFileSync(join(f.cwd, "file.txt"), "pending change\n");
    const result = f.run(["--pr", "-y"], { TEST_PR: "1", ...extra });
    assert.equal(result.status, 1);
